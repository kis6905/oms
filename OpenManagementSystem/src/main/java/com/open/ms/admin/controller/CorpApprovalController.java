 ...
 */
@Controller
@RequestMapping(value = "/admin/corp/approval/**")
public class CorpApprovalController {
	
	private static final Logger logger = LoggerFactory.getLogger(CorpApprovalController.class);
	...
	public String postList(
		...
		try {
			...
			List<Approval> approvalList = corpApprovalServiceImpl.getApprovalList(null, null, startDate, endDate, statusCode, offset, limit, sort, order);
			for (Approval vo : approvalList)
				rows.add(vo.toJSONObject());
			
			int totalCnt = corpApprovalServiceImpl.getApprovalListTotalCnt(null, null, startDate, endDate, statusCode);
