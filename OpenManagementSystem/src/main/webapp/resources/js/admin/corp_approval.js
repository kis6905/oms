...
var table, detailcorpMoneybookTable;
var canvas, ctx;
var detailRow;
...
$(document).ready(function() {
	...
	$('#corpMoneybookDetailCloseBtn').on('click', function() {
		$('#corpMoneybookDetailModal').modal('hide');
	});
...
function openDetailModal(row) {
	...
	$('#corpMoneybookDetailModal').modal();
	$('#corpMoneybookDetailTitle').html(row.title);
	
	var statusHtml = getLabelOfStatusCode(row.statusCode);
	if (row.statusCode != 1201)
		statusHtml += '&nbsp;&nbsp;처리 일시: ' + row.completedDate;
	$('#corpMoneybookDetailStatus').html(statusHtml);
	
	$('#corpMoneybookDetailTerm').html(row.startDate + ' ~ ' + row.endDate);
	...
	if (typeof detailTable === 'undefined' || detailTable === null)
		createCorpMoneybookDetailTable(data);
	else
		reCreateCorpMoneybookDetailTable(data);
}
...
/**
 * 법 결재 상세보기 테이블 제거 후 생성
 */
function reCreateCorpMoneybookDetailTable(data) {
	if (typeof detailTable !== 'undefined' || detailTable !== null) {
 ...
 * 법카 결재 상세보기 테이블 생성
 */
function createCorpMoneybookDetailTable(data) {
	// 테이블 생성
	detailTable = $('#corpMoneybookTable').bootstrapTable({
		ajaxOptions: ajaxOption,
		method: 'post',
...
//			location.href = '#';
		...
		onLoadSuccess: function(data) {
			$('#MoneybookDetailTotalCnt').html(data.total);
			var totalPrice = data.totalPrice == null ? 0 : data.totalPrice;
			$('#corpMoneybookDetailPayment').html(numberFormat(totalPrice));
		},
		onLoadError: btOnLoadErrorHandler,
		columns: [{
			field: 'no',
...
function downloadExcel() {
	...
	$form.attr('method', 'post')
		.attr('action', '/service/corp/moneybook/approval/excel')
		.append($(sortInput))
		.append($(orderInput))
		.append($(seq))
		.submit();
