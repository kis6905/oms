...
//            return 'No matching records found';
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        /**
    	 * Custom source
    	 * search input 옆에 검색 버튼 추가를 위해 커스터마이징한 소스이다.
    	 * 밑에 BootstrapTable.prototype.onSearch와 세트로 맞춰줘야 한다.
    	 * 
    	 * @author iskwon
    	 */
        if (this.options.search) {
        	html = [];
            html.push(
            		sprintf('<div class="columns columns-%s btn-group pull-%s">', this.options.buttonsAlign, this.options.buttonsAlign),
            		'<button class="btn btn-default' + (this.options.iconSize === undefined ? '' : ' btn-' + this.options.iconSize) + '" type="button" name="searchBtn" title="Search">',
            ...
            this.$toolbar.find('button[name="searchBtn"]')
            .off('click').on('click', function () {
//            	$.proxy(this.refresh, this);
	 ...
	 * Custom source
	 * search input 옆에 검색 버튼 추가를 위해 커스터마이징한 소스이다.
	 * 위에 if (this.options.search) {...} 의 내용과 세트로 맞춰줘야 한다.
	 * 
	 * @author iskwon
	 */
    BootstrapTable.prototype.onSearch = function (event) {
    	var text = $.trim(this.$toolbar.find('.search input').val());

        // trim search input
        if (this.options.trimOnSearch && this.$toolbar.find('.search input').val() !== text) {
        	this.$toolbar.find('.search input').val(text);
        }
...
//        html.push(this.options.formatRecordsPerPage(pageNumber.join('')));
        ...
        for (i = from; i <= to; i++) {
        	this.options.pageNumber = this.options.pageNumber * 1; // custom source, === 연산자 때문에 data type을 맞춰줌
